...
> Existing tools work at one level — either code smells OR architecture. I bridge both: code-level smells from DesigniteJava (like God Class and Feature Envy) become signals for architectural-level anti-patterns (God Service and Wrong Cuts). For example, a single God Class flags the whole service as a God Service.
...
**Q: Explain the two-signal God Service detection.**
> First signal: query DesigniteJava for God Class smells — one is enough to flag the service. If DesigniteJava found none, I fall back to a **Spoon structural analysis** over six metrics per class (field count, public methods, LOC, import domains, constructor params, and TCC — Tight Class Cohesion). A class is a God Class if it exceeds at least 3 of the 6. Pure data-holder classes (entities, DTOs) are excluded first so they don't get falsely flagged.
...
> Two signals. First, DesigniteJava God Class smells — one is enough. If DesigniteJava found none, I fall back to a **Spoon** structural analysis over six per-class metrics (≥25 fields, ≥30 public methods, ≥1000 LOC, ≥20 import domains, ≥12 constructor params, TCC < 0.5); a class is a God Class if it exceeds at least **3 of the 6**. Pure data holders (entities, DTOs, getter/setter classes) are excluded first.
...
> Two indicators. First, **Feature Envy** smell count ≥ **3** in a service (configurable) — methods reaching into other classes' data suggest misplaced logic. Second, **bidirectional dependencies**: a pair of services calling each other in both directions, which signals they're too tightly coupled to be separate. Each pair is reported once, with the call declarations from both directions as evidence.
...
| God Service | HIGH | ≥1 God Class, or ≥3 of 6 structural metrics |
...
| Wrong Cuts | HIGH | ≥3 Feature Envy, or bidirectional deps |
