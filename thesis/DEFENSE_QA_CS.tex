...
\title{\textbf{Thesis Defense --- Q\&A Cheat Sheet}}
\author{}
\date{}
...
\begin{description}
    ...
    A system-level check using three metrics: \textbf{coupling coefficient} \(C\), \textbf{connected ratio} \(R\), and \textbf{shared-database count} \(D\). I flag the system if
    ...
    Only runs with at least 3 services.
...
\begin{description}
    ...
    \textbf{Code Quality (20):} smell density per KLOC, scaled:

    $$\min(20, \operatorname{round}(20 \times \text{density} / 80))$$

    \textbf{Architecture (25):} coupling coefficient above 0.1 costs

    $$\min(15, \operatorname{round}(\text{coupling} \times 15))$$

    plus

    $$\min(10, 5 \text{ per cycle})$$
