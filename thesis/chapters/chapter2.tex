...
Second, the tool offers broad anti-pattern coverage across several architectural dimensions, detecting ten distinct anti-patterns: service design (Nano Service, God Service), inter-service communication (Chatty Service, Cyclic Dependency, Hardcoded Endpoints, ESB misuse), data management (Shared Database), and system-level coupling (Distributed Monolith, API Versioning Absence, Wrong Cuts). Compared with the existing tools reviewed in this chapter, this work emphasizes breadth across both service-level and system-level concerns, while also combining the detected architectural findings with code-level smell evidence.
