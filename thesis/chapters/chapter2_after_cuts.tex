...
The detection of nano services requires defining quantitative thresholds. In this work, a service is classified as a nano service when its lines of code fall below a configurable threshold (default: 500) and it exposes no more than a configurable number of endpoints (default: 2). These thresholds are configurable and should be calibrated based on the specific project context. The precise detection criterion is formalised in Chapter~\ref{chap:ch3}.
...
Detection of god services in this work is based on the presence of code-level indicators. Specifically, a microservice is flagged as a god service when the number of God Class code smells it contains (as detected by DesigniteJava) meets or exceeds a configurable threshold (default: 1). The detailed detection procedure, including a structural fallback analysis, is described in Chapter~\ref{chap:ch3}.
...
Detection involves analyzing the dependency graph and identifying edges whose call count meets or exceeds a configurable threshold (default: 10).
...
A distributed monolith typically arises when services share too many dependencies, communicate excessively, or rely on synchronous call chains to fulfill requests. The coupling coefficient $C$, which is the ratio of actual to possible inter-service dependencies, defined formally in Chapter~\ref{chap:ch3}, serves as a quantitative indicator: when a large proportion of all possible inter-service dependencies actually exist, the system behaves more like a monolith rather than a suite of decoupled services, each maintaining a high degree of operational independence. In this work, a system is flagged as a potential distributed monolith when this coupling coefficient exceeds a configurable threshold.
...
\par This dissertation addresses the gaps identified in the reviewed literature in several ways. Unlike existing tools, which typically operate at a single level of abstraction (either code-level smell detection such as DesigniteJava, or architectural-level dependency analysis such as Arcan and MicroART), this work performs multi-level analysis, combining intra-service code smell detection (via DesigniteJava) with inter-service architectural analysis (via Spoon-based Abstract Syntax Tree analysis and graph algorithms). For instance, God Class smells detected at the code level are used as evidence for flagging god services at the architectural level. It offers broader anti-pattern coverage, detecting ten distinct anti-patterns across four architectural dimensions, more than any single tool reviewed in this chapter, and it provides automated microservice boundary detection through a \textit{deployability gate} that identifies independently deployable services from build files, eliminating the manual service enumeration required by most existing tools.

\par In addition, detection thresholds for metrics-based anti-patterns are externalized as configurable parameters that can be calibrated to project-specific norms. Moreover, a composite health score provides a single numeric metric (0-100) with letter grading, decomposed into four weighted categories (anti-pattern severity, code quality, architectural coupling and service sizing) so that teams can identify which quality dimension is degraded and track it over time. Furthermore, every detected anti-pattern is accompanied by source code snippets and remediation guidance rather than an abstract warning. Finally, the tool is delivered as a web application with a RESTful API, supporting both interactive use through a dashboard and programmatic integration into CI/CD pipelines. These contributions, outlined in the introduction, are elaborated in the chapters that follow.

\section{Summary}
\label{sec:summary}

\par This chapter has established the theoretical foundation for this dissertation. The microservices architectural style was introduced along its core principles, inherent challenges and a taxonomy of ten microservice anti-patterns.
