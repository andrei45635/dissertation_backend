...
The first and strongest signal is the framework entry point (HIGH confidence). The module's \texttt{src/main/java} tree is scanned for a class annotated with a known framework startup annotation: \texttt{@Spring\-Boot\-Application}, \texttt{@QuarkusMain}, \texttt{@ApplicationPath}, or a call to \texttt{Micronaut.run()}. Files whose names end in \texttt{Application.java} or \texttt{Main.java} are checked first as a fast path before falling back to a full directory scan.
