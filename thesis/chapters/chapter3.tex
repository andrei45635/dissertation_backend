...
\noindent where $n_{\mathrm{crit}}$, $n_{\mathrm{high}}$, $n_{\mathrm{med}}$, and $n_{\mathrm{low}}$ represent the number of detected anti-patterns at each severity level. This scoring mechanism provides a quick assesment that can be tracked over time to monitor architectural health trends.
...
As evidence, the detector locates the controller classes that declare unversioned endpoints and extracts the code snippets showing the \texttt{@RestController} or \texttt{@Re\-questMap\-ping} annotations. This allows the developer to see where versioning prefixes (e.g., \texttt{/v1/}) could be added.
