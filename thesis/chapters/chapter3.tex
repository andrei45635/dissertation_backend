...
\par Static code analysis examines source code without executing it, allowing detection of structural issues, code smells, and architectural violations at the source level \cite{Chess2007}. In the context of microservices, static analysis can be applied at two levels. Intra-service analysis examines the internal structure of each microservice for code-level smells such as God Class, Long Method, and Complex Method \cite{Fowler1999}. The aforementioned smells serve as indicators of architectural problems at the service level. Inter-service analysis, on the other hand, examines the communication patterns, dependencies, and shared resources across microservices to detect architectural anti-patterns such as cyclic dependencies and shared databases.
...
\par DesigniteJava is a static analysis tool that detects code smells and computes object-oriented metrics for Java projects \cite{Sharma2024}. It analyzes Java source code and produces CSV reports categorizing detected smells by type, affected class and file location.

In this dissertation, DesigniteJava is integrated as an external tool invoked as a subprocess. The analysis pipeline passes each detected microservice's source directory to DesigniteJava, parses the available smell-report CSV files produced by the configured installation, and stores the results in the application database. The aggregate density of these smells per service is then used as the code-quality signal that feeds the composite health score (see Section~\ref{subsec:health-score}).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|p{5.5cm}|l|}
        ...
        \hline\hline
        \multirow{6}{*}{Design} & Unutilized Abstraction & Abstraction that is created but never used & Low \\
        \cline{2-4}
        & Unnecessary Abstraction & Abstraction introduced without a clear purpose & Low \\
        \cline{2-4}
        & Deficient Encapsulation & Accessibility more permissive than required & Medium \\
        \cline{2-4}
        & Broken Modularization & Data and behaviour that belong together are split apart & Medium \\
        \cline{2-4}
        & Cyclic-Dependent Modularization & Abstractions that depend on each other cyclically & High \\
        \cline{2-4}
        & Broken Hierarchy & Supertype and subtype without a genuine is-a relationship & Medium \\
        \hline
        \multirow{5}{*}{Implementation} & Complex Method & Method with high cyclomatic complexity & Medium \\
        \cline{2-4}
        & Complex Conditional & Conditional expression that is difficult to follow & Medium \\
        \cline{2-4}
        & Long Statement & Excessively long single statement & Low \\
        \cline{2-4}
        ...
        \hline
    \end{tabular}
    \caption{Representative code smells reported by DesigniteJava and observed across the evaluated projects, grouped by category. All parsed smells contribute to the overall smell density used by the health score.}
...
\par The dependency graph is a directed graph $G = (V, E)$ where each vertex $v \in V$ represents a microservice and each edge $e = (s, t) \in E$ represents a dependency from service $s$ to service $t$. Edges are annotated with metadata including the dependency type (REST synchronous, Feign client, etc.) and the call count.
...
\par The first step is a configuration pre-scan: before scanning source code, each service's \texttt{application.yml} or \texttt{application.properties} is parsed. The \texttt{spring\-.application.name} property is registered as an alias for the service directory name, and \texttt{server.port} values are recorded in a port-to-service mapping. Next, the pipeline performs \texttt{@Value} field resolution, in which fields annotated with Spring's \texttt{@Value} (e.g., \texttt{@Value("\$\{order\--service.url:http://localhost:8081\}")}) are pre-scanned using Spoon. Property placeholders are resolved against the service's configuration, falling back to default values when the property key is not found. Finally, a cascading target resolution strategy is applied. For declarative Feign clients, the builder first extracts the logical service identifier from the \texttt{@Feign\-Client} annotation, such as the \texttt{name} or \texttt{value} attribute, and matches it against detected service names and registered aliases. For URL-based targets, such as \texttt{Rest\-Template} and \texttt{WebClient} calls, the builder also supports port-based matching for \texttt{local\-host:PORT}-style targets. If no exact or port-based match is found, fuzzy substring matching with common suffix stripping is applied (e.g., matching ''order'' to ''order-service''). For \texttt{@FeignClient} interfaces, each non-default, non-static method is registered as a separate call evidence entry, since each method declares a distinct remote call site. This yields a per-method call count that feeds into chatty service detection as a static proxy for runtime call frequency.
...
\par The first category, Anti-Patterns ($M_{\mathrm{ap}} = 40$), penalises each detected anti-pattern according to its severity (Table~\ref{tab:severity-weights}). The deduction per issue is 8~points for critical severity, 5 for high, 3 for medium, and 1 for low. Nano Service and God Service instances are excluded from this category and accounted for solely under Service Sizing, so that each service-sizing issue contributes to the score exactly once.

\par The second category, Code Quality ($M_{\mathrm{cq}} = 20$), captures the \emph{density} of code smells reported by DesigniteJava, expressed as smells per thousand lines of code (KLOC) so that large codebases are not penalised merely for their size. Let $\rho = n_{\mathrm{smells}} / (\mathit{LOC}/1000)$ denote the smell density. The penalty scales linearly with density and reaches the full budget at a threshold $\rho_{\max}$ (default 80 smells/KLOC, configurable via \texttt{app.thresholds.code-smell-density-threshold}), $P_{\mathrm{cq}} = \min\bigl(20,\; \lfloor 20 \cdot \rho / \rho_{\max} \rceil\bigr)$. Scoring density rather than absolute count prevents a large but proportionally clean codebase from being penalised as heavily as a small, smell-dense one.
...
\par The final score is clamped to $[0, 100]$ and mapped to a letter grade: A~($\geq 90$), B~($\geq 80$), C~($\geq 65$), D~($\geq 50$), F~($< 50$). Because the deductions are itemised per category, a reader can see which quality dimension contributes most to a low score and compare the per-category scores across analysis runs.

Table~\ref{tab:severity-weights} details the severity levels, their corresponding penalty weights in the anti-pattern category, and examples of anti-patterns assigned to each level within that category. Nano Service and God Service still retain severities in the generated reports, but they are scored exclusively under the Service Sizing category described above.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|c|p{6.5cm}|}
        \hline
        \textbf{Severity} & \textbf{Penalty} & \textbf{Example Anti-Patterns} \\
        \hline\hline
        Critical & $-8$ per issue & Cyclic Dependency, Distributed Monolith \\
        \hline
        High & $-5$ per issue & Shared Database, Wrong Cuts, Chatty Service, ESB Misuse \\
        \hline
        Medium & $-3$ per issue & Hardcoded Endpoints, API Versioning Absence \\
        \hline
        Low & $-1$ per issue & --- \\
        \hline
    \end{tabular}
    \caption{Severity levels with their penalty deductions in the anti-pattern category of the health score computation (Equation~\ref{eq:health-score}), excluding service-sizing findings.}
    \label{tab:severity-weights}
\end{table}

\subsection{Microservice Boundary Detection}
\label{subsec:microservice-detection}

\par Accurate identification of microservice boundaries within a multi-module project is a prerequisite for all downstream analyses. Rather than treating every directory containing a build file as a microservice, the detector applies a \textit{deployability gate}: a three-signal heuristic that verifies whether a candidate module is an independently deployable service.

The detection proceeds in two phases. In the candidate discovery phase, the project directory is recursively scanned for build files (\texttt{pom.xml}, \texttt{build.gradle}, \texttt{build\-.gradle\-.kts}). Each directory containing at least one build file becomes a candidate, subject to two filtering steps. First, directories whose name matches an exclusion keyword (e.g.\ \texttt{example}, \texttt{test}, \texttt{benchmark}, \texttt{doc}, \texttt{bom}), or whose ancestor directory does, are discarded. The exclusion set covers common non-service module names related to examples, samples, documentation, test harnesses, build support modules, starters and templates. Second, aggregator modules are identified and excluded: the project root is treated as an aggregator when it contains \texttt{<modules>} declarations (Maven) or \texttt{settings.gradle} with \texttt{include} statements (Gradle). Nested modules that declare their own submodules are likewise excluded.

In the deployability gate phase, each surviving candidate is evaluated against three signals, applied in priority order.
The first and strongest signal is the framework entry point (HIGH confidence). The module's \texttt{src/main/java} tree is scanned for a class annotated with a known framework startup annotation, such as \texttt{@Spring\-Boot\-Application}, \texttt{@QuarkusMain}, or \texttt{@ApplicationPath}, or one that contains a call to \texttt{Micronaut.run()}. Files whose names end in \texttt{Application.java} or \texttt{Main.java} are checked first as a fast path before falling back to a full directory scan.
The second signal is Dockerfile presence (MEDIUM confidence). The module's root directory is checked for a \texttt{Dockerfile}, \texttt{dockerfile}, or any file matching the pattern \texttt{*.Dockerfile}. A Dockerfile implies an independently deployable container, even in the absence of a recognised framework annotation.
The third and weakest signal is the presence of a \texttt{main()} method (LOW confidence). The module's \texttt{src/main/java} tree is scanned for a \texttt{public static void main(String[])} method signature. This is the weakest signal, as many non-service modules — such as CLI tools or batch jobs — contain a main method without representing a deployable microservice.

A candidate that does not match any of the three signals is discarded. When the gate eliminates all candidates, three fallback strategies are applied in order: if there was exactly one candidate, it is retained with LOW confidence. Should no build files be found at all, the project root is used. Otherwise, the project root is used as a catch-all fallback.

Each detected service is assigned a \texttt{DetectionConfidence} level (HIGH, MEDIUM, or LOW) and a human-readable signal label (e.g.\ \texttt{framework-entry-point}, \texttt{docker\-file}, \texttt{main-method}), which are persisted alongside the microservice entity. This metadata enables users to understand \textit{why} each module was classified as a service and to assess detection quality.

The deployability gate is designed to reduce false positives compared to a build-file-only heuristic, particularly in large monorepository projects where utility libraries, shared kernels, and test modules all contain build files but are not independently deployable services.

\section{Per-Anti-Pattern Detection Methodology}
\label{sec:per-anti-pattern-detection}

\par This section describes the detection algorithm for each anti-pattern targeted by this work. For each anti-pattern, the input data, detection logic, configurable thresholds, and evidence collection mechanism are presented. The definitions and motivations for each anti-pattern were introduced in Chapter~\ref{chap:ch2}, while this section focuses on the \textit{how} of detection.
...
In the first phase, the configuration files of each microservice are parsed to extract the datasource URL. For Spring Boot applications, the detector reads \texttt{app\-li\-ca\-tion.yml}, \texttt{application.yaml}, or \texttt{application.properties} configuration files located in the \texttt{/resources} directory of each service. From YAML files, the nested property \texttt{spring.datasource.url} is extracted using a recursive map traversal, whereas from properties files, the same key is read directly. Spring property placeholders with default values (e.g., \texttt{\$\{DB\_HOST:localhost\}}) are resolved to their defaults to enable meaningful comparison.
...
\par The Nano Service detector identifies services whose size is too small to justify the operational overhead of running them as independent microservices \cite{Tighilt2023}. Detection is based on two configurable metrics: the lines of code (LOC) and the number of REST endpoints exposed by the service.
...
\noindent where $\theta_{\mathit{LOC}}$ is the maximum lines of code threshold (default: 500, configurable via \texttt{app.\-thresh\-olds.\-nano-service\--max\--loc}) and $\theta_{\mathit{ep}}$ is the maximum endpoint count threshold (default: 2, configurable via \texttt{app.thresholds.nano-service\--max-endpoints}).

The LOC count is computed during the microservice detection phase by recursively walking through the service's directory tree and counting lines in all \texttt{.java} files. The endpoint count is populated during the inter-service analysis phase via Spoon-based annotation scanning.
...
\par The God Service detector identifies microservices that handle too many responsibilities \cite{Brown1998}. A service is flagged as a god service when it contains at least one class that exhibits God Class characteristics:

\begin{equation}
    \exists\, c \in \mathit{classes}(s) : \mathit{isGodClass}(c)
    \label{eq:god-service-detection}
\end{equation}

\noindent A single God Class is sufficient, since even one class that concentrates a large share of a service's responsibilities undermines its cohesion and impairs independent deployability and testability. Whether a class is a God Class is determined by a Spoon-based structural analysis of every class in the service. Before scoring, pure data-holder classes (those carrying a data annotation such as \texttt{@Entity} or \texttt{@Data}, having a data-class name suffix such as \texttt{Dto} or \texttt{Model}, or whose public methods are predominantly getters and setters) are excluded, since such classes naturally exhibit many fields and low cohesion without representing an architectural anti-pattern. Each remaining non-abstract class is evaluated against six metrics, summarised in Table~\ref{tab:god-class-metrics}. A class is flagged as a God Class when $\geq 3$ of the six metrics simultaneously exceed their thresholds:
...
This structural approach highlights the multi-level analysis contribution of this work, since code-level metrics such as class cohesion are used as signals for an architectural-level anti-pattern.

\subsection{Chatty Service Detection}
\label{subsec:detect-chatty-service}

\par The Chatty Service detector identifies services that communicate excessively through fine-grained remote calls \cite{Taibi2018, Richardson2018, AzureChattyIO}. Detection uses two complementary approaches.
...
\noindent where $\theta_{\mathit{chatty}}$ defaults to~5 (configurable via \texttt{app.\-thres\-holds.\-cha\-tty-ser\-vice\--min\--calls}). During dependency graph construction, each non-default method in a \texttt{@FeignClient} interface is counted as a separate call (rather than one call per annotation), so a Feign interface with 50~methods produces a call count of~50 on the corresponding edge.
...
The detector uses a URL-literal regular expression to identify hardcoded endpoints. It matches HTTP and HTTPS URLs, \texttt{localhost:PORT}, \texttt{127.0.0.1:PORT}, and explicit IP-address endpoints such as \texttt{10.0.0.5:8080}.
For each match, the detector verifies that the match occurs within a string literal (i.e., enclosed in quotes) before recording it as evidence. Several categories of lines are excluded from the scan to reduce false positives: comment lines (starting with \texttt{//}, \texttt{*}, or \texttt{/*}), import statements (\texttt{import...}), package declarations (\texttt{package...}), and test files (paths containing \texttt{test} or \texttt{Test}).
...
During endpoint extraction, each detected endpoint is checked for the presence of a versioning strategy, and a boolean flag \texttt{hasVersioning} is set on the endpoint entity accordingly. Four complementary strategies are recognised. The first is URL path versioning, where the endpoint's full path, formed by concatenating the class-level \texttt{@RequestMapping} prefix with the method-level mapping, is matched against the regular expression \texttt{/v(\textbackslash d+)([/.]\textbar\$)}, which captures path segments such as \texttt{/v1/}, \texttt{/v2/}, \texttt{/v1.}, or a trailing \texttt{/v1}. The second is header versioning, for which the mapping annotation's \texttt{headers} attribute is inspected for a version header such as \texttt{X-API-Version}, \texttt{api-version}, or \texttt{accept-version}. The third is media-type, or content-negotiation, versioning, where the \texttt{produces} and \texttt{consumes} attributes are inspected for a vendor media type that embeds a version, for example \texttt{application/vnd.example.v1+json}. The fourth is query-parameter versioning, for which the \texttt{params} attribute is inspected for a version query parameter such as \texttt{version=} or \texttt{v=}. An endpoint is considered versioned when any of these strategies matches, with URL path versioning being the most common in practice.
...
\par The Wrong Cuts detector identifies misplaced service boundaries where functionality that should reside in one service is spread across multiple services, or tightly-coupled functionality has been incorrectly split \cite{Taibi2018}. The detector relies on bidirectional dependencies between services as its indicator. It scans the dependency graph for pairs of services with mutual (bidirectional) dependencies (i.e., edges in both directions between the same pair of vertices). Bidirectional dependencies suggest that the two services are too tightly coupled and may have been incorrectly separated:
...
The detector computes three signals for each service. Let $s$ be a service, $\text{callers}(s)$ the set of services that have a dependency edge targeting $s$, $\text{callees}(s)$ the set of services that $s$ depends on, and $n = |V| - 1$ the number of other services:
...
The third signal is \textit{betweenness centrality}, a graph-theoretic measure of how frequently a node lies on shortest paths between other node pairs \cite{Brandes2001}. For a node $v$ in directed graph $G = (V, E)$:
...
\noindent where $\sigma_{st}$ is the total number of shortest paths from $s$ to $t$, and $\sigma_{st}(v)$ is the number of those paths that pass through $v$. To enable threshold comparison across graphs of different sizes, the raw score is normalised by the maximum possible value, which for a directed graph over the full set of $|V|$ services is $(|V|-1)(|V|-2)$:

\begin{equation}
    BC_{\mathit{norm}}(v) = \frac{BC(v)}{(|V|-1)(|V|-2)}
    \label{eq:betweenness-norm}
\end{equation}

\noindent Betweenness centrality is computed using Brandes' algorithm \cite{Brandes2001}, which runs in $O(|V| \cdot |E|)$ time and is well-suited to the small-to-medium dependency graphs typical of microservice systems.

A service is flagged as an ESB-like mediator when any of the following three conditions holds:

\begin{equation}
    \begin{array}{c}
        (\mathit{callerRatio}(s) \geq \theta_m \;\wedge\; \mathit{calleeRatio}(s) \geq \theta_m) \\[4pt]
        \vee\;\; \mathit{medRatio}(s) \geq \theta_m \\[4pt]
        \vee\;\; BC_{\mathit{norm}}(s) \geq \theta_m
    \end{array}
    \label{eq:esb-decision}
\end{equation}

\noindent where $\theta_m$ is the mediator threshold (default: 0.4, configurable via \texttt{app.thres\-h-olds.esb-mediator-threshold}). The first condition identifies services that both receive calls from and make calls to a high proportion of other services. The second condition captures cases where a single service concentrates a disproportionate share of all dependency traffic by volume. The third condition flags services that act as structural intermediaries on the shortest communication paths between other services, even when connection and volume ratios alone may not exceed the threshold. Services whose names contain gateway-related keywords (e.g.\ \texttt{gateway}, \texttt{api-gateway}, \texttt{zuul}, \texttt{proxy}) are excluded, as they are architecturally designed to be central.
...
\par This chapter presented the detection methodology employed by this work. The analysis pipeline was described, starting from source code ingestion, through microservice boundary detection via a three-signal deployability gate (Section~\ref{subsec:microservice-detection}), intra-service code smell analysis (via DesigniteJava) and inter-service dependency extraction (via Spoon-based AST analysis), to dependency graph construction and anti-pattern detection.

The detailed detection algorithm for each of the ten targeted anti-patterns was presented. Shared Database detection compares datasource URLs across services (Section~\ref{subsec:detect-shared-database}). Cyclic Dependency detection applies Tarjan's SCC algorithm on the dependency graph (Section~\ref{subsec:detect-cyclic-dependency}). Nano Service detection uses LOC and endpoint count thresholds (Section~\ref{subsec:detect-nano-service}). God Service detection applies a Spoon-based multi-metric structural analysis of each class (Section~\ref{subsec:detect-god-service}). Chatty Service detection analyzes both dependency edge call counts and source-based HTTP~client patterns (Section~\ref{subsec:detect-chatty-service}). Hardcoded Endpoint detection performs URL pattern matching in source files (Section~\ref{subsec:detect-hardcoded-endpoint}). Distributed Monolith detection employs composite coupling metric analysis (Section~\ref{subsec:detect-distributed-monolith}). API Versioning Absence detection matches endpoint path versioning patterns (Section~\ref{subsec:detect-versioning-absence}).
Wrong Cuts detection identifies bidirectional dependencies between service pairs (Section~\ref{subsec:detect-wrong-cuts}). Finally, ESB Misuse detection uses a three-signal approach combining connection ratios, volume-based mediator ratio, and normalised betweenness centrality (Section~\ref{subsec:detect-esb-misuse}).
