...
The deployability gate significantly reduces false positives compared to a build-file-only heuristic, particularly in large monorepository projects where utility libraries, shared kernels, and test modules all contain build files but are not independently deployable services.
...
\par The God Service detector identifies microservices that handle too many responsibilities \cite{Taibi2018}. A service is flagged as a god service when it contains at least one class that exhibits God Class characteristics:
