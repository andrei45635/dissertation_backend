...
\indent \par This section outlines the techniques used for detecting architectural anti-patterns in microservice systems. Figure~\ref{fig:analysis-pipeline} provides an overview of the analysis pipeline, showing how source code is processed through multiple stages to produce anti-pattern detection results.
...
\indent\par Static code analysis examines source code without executing it, allowing detection of structural issues, code smells, and architectural violations at the source level \cite{Chess2007}. In the context of microservices, static analysis can be applied at two levels. Intra-service analysis examines the internal structure of each microservice for code-level smells such as God Class, Long Method, Feature Envy, and Complex Method; these smells serve as indicators of architectural problems at the service level. Inter-service analysis, on the other hand, examines the communication patterns, dependencies, and shared resources across microservices to detect architectural anti-patterns such as cyclic dependencies and shared databases.

\subsection{Abstract Syntax Tree Analysis with Spoon}
\label{subsec:spoon-analysis}

\indent \par Spoon is an open-source library for analyzing, transforming and generating Java source code \cite{Pawlak2016}. It builds a fine-grained Abstract Syntax Tree (AST) that provides full access to all elements of the Java programming language, including annotations, method invocations and type references.
...
\indent \par DesigniteJava is a static analysis tool that detects design smells, implementation smells and architecture smells in Java projects \cite{Sharma2018}. It analyzes Java source code and produces CSV reports categorizing detected smells by type, affected class and file location. 
...
\indent \par To provide an overall assessment of the system's architectural health, this dissertation computes a composite health score that combines multiple quality dimensions into a single 0--100 metric. Rather than applying a flat severity-weighted deduction, the score budget is partitioned into four complementary categories, each capturing a distinct aspect of microservice quality. This category-based approach gives teams an at-a-glance numeric score while also providing a detailed breakdown that highlights \textit{where} the score was lost.

\subsubsection{Score Categories}
\label{subsubsec:score-categories}

The total budget of 100 points is distributed across four categories as shown in Table~\ref{tab:score-categories}.
...
\hline
\textbf{Category} & \textbf{Max Points} & \textbf{What It Measures} \\
\hline\hline
Anti-Patterns & 40 & Detected architectural anti-patterns, weighted by severity \\
\hline
Code Quality & 20 & Code smell density from static analysis (DesigniteJava) \\
\hline
Architecture & 25 & Inter-service coupling coefficient and dependency cycles \\
\hline
Service Sizing & 15 & Appropriateness of individual service sizes (Nano / God) \\
\hline
\end{tabular}
\caption{Health score categories and their point budgets.}
\label{tab:score-categories}
\end{table}

The overall health score is the sum of the four category scores:

\begin{equation}
    H = \sum_{k=1}^{4} S_k, \qquad S_k = \max\!\big(0,\; M_k - P_k\big)
    \label{eq:health-score}
\end{equation}

\noindent where $M_k$ is the maximum point budget for category $k$ and $P_k$ is the total penalty deducted from that category. Each category computes its penalty $P_k$ using a different deduction model, as described below.

\subsubsection{Anti-Pattern Category (40 points)}
\label{subsubsec:cat-antipatterns}

Each detected anti-pattern incurs a severity-weighted penalty. Let $A$ be the set of all detected anti-patterns and $w(a)$ the severity weight of anti-pattern $a$:

\begin{equation}
    P_{\mathrm{ap}} = \sum_{a \in A} w(a)
    \label{eq:cat-antipatterns}
\end{equation}

Table~\ref{tab:severity-weights} lists the per-occurrence penalty for each severity level.
...
\hline
\textbf{Severity} & \textbf{Penalty $w(a)$} & \textbf{Example Anti-Patterns} \\
\hline\hline
Critical & 8 & Cyclic Dependency, Distributed Monolith \\
\hline
High & 5 & Shared Database, God Service, Chatty Service \\
\hline
Medium & 3 & Nano Service, Hardcoded Endpoints, API Versioning Absence \\
\hline
Low & 1 & --- \\
\hline
\end{tabular}
\caption{Severity levels and their per-occurrence penalty in the Anti-Pattern category.}
\label{tab:severity-weights}
\end{table}

Anti-patterns are grouped by type in the breakdown, so a user can see, for example, ``Cyclic Dependency ($\times$2): $-$10'' rather than an opaque aggregate.

\subsubsection{Code Quality Category (20 points)}
\label{subsubsec:cat-codequality}

Code smell density is penalized using a logarithmic scale so that the first smells incur a proportionally larger penalty, with diminishing marginal impact as the count grows:

\begin{equation}
    P_{\mathrm{cq}} = \min\!\Big(20,\; \big\lfloor \ln(1 + n_{\mathrm{smells}}) \times 3.5 \big\rfloor\Big)
    \label{eq:cat-codequality}
\end{equation}

\noindent where $n_{\mathrm{smells}}$ is the total number of code smells detected across all microservices by DesigniteJava.

\subsubsection{Architecture Category (25 points)}
\label{subsubsec:cat-architecture}

This category penalizes two structural indicators:

\begin{enumerate}
    \item \textbf{Coupling coefficient} ($C$, see Equation~\ref{eq:coupling-coefficient}): values above $0.1$ incur a penalty of $\min(15,\; \lfloor C \times 15 \rfloor)$ points.
    \item \textbf{Dependency cycles}: each cycle detected by Tarjan's algorithm (Section~\ref{subsec:detect-cyclic-dependency}) incurs a penalty of 5 points, capped at 10 points total.
\end{enumerate}

\begin{equation}
    P_{\mathrm{arch}} = \min(15,\; \lfloor C \times 15 \rfloor)\;\cdot\;\mathbb{1}_{[C > 0.1]} \;+\; \min(10,\; 5\,n_{\mathrm{cycles}})
    \label{eq:cat-architecture}
\end{equation}

\subsubsection{Service Sizing Category (15 points)}
\label{subsubsec:cat-sizing}

This category targets the two service-sizing anti-patterns:

\begin{itemize}
    \item \textbf{Nano services}: 3 points per occurrence, capped at 8.
    \item \textbf{God services}: 5 points per occurrence, capped at 10.
\end{itemize}

\begin{equation}
    P_{\mathrm{sz}} = \min(8,\; 3\,n_{\mathrm{nano}}) + \min(10,\; 5\,n_{\mathrm{god}})
    \label{eq:cat-sizing}
\end{equation}

\subsubsection{Letter Grade}
\label{subsubsec:grade}

To complement the numeric score, a letter grade provides a quick qualitative assessment. Table~\ref{tab:grade-scale} defines the mapping.
...
\hline
\end{tabular}
\caption{Letter grade mapping for the composite health score.}
\label{tab:grade-scale}
\end{table}

The breakdown response returned by the API includes the overall score, the letter grade, and for each category: its name, description, score, maximum score, and a list of deductions (each with a human-readable reason, the occurrence count, and the number of points lost). This structured format enables the frontend to render the score as a set of collapsible category cards with progress bars, providing developers with immediate insight into which quality dimensions require attention.
...
\indent \par The Shared Database detector operates by comparing the datasource connection URLs configured across all detected microservices. The algorithm proceeds in two phases. 

In the first phase, the configuration files of each microservice are parsed to extract the datasource URL. For Spring Boot applications, the detector reads \texttt{application.yml}, \texttt{application.yaml}, or \texttt{application.properties} configuration files located in the \texttt{/resources} directory of each service. From YAML files, the nested property \texttt{spring.datasource.url} is extract using a recursive map traversal, whereas from properties files, the same key is read directly. Spring propery placeholders with default values (e.g., \texttt{\$\{DB\_HOST:localhost\}}) are resolved to their defaults to enable meaningful comparison.

In the second phase, microservices are grouped by their resolved datasource URL. Any group containing more than one service indicates a shared database instance. Formally, let $\mathcal{S}$ denote the set of all microservices and $\text{dsUrl}$ denote the resolved datasource URL of service $s$. The detector reports a Shared Database anti-pattern for each equivalence class:
...
\indent \par The Nano Service detector identifies services whose size is too small to justify the operational overhead of running them as independent microservices \cite{Taibi2018}. Detection is based on two configurable metrics: the lines of code (LOC) and the number of REST endpoints exposed by the service.
...
\noindent where $\theta_{\mathit{LOC}}$ is the maximum lines of code threshold (default: 500, configurable via \texttt{app.thresholds.nano-service-max-loc}) and $\theta{\mathit{ep}}$ is the maximum endpoint count threshold (default: 2, configurable via \texttt{app.thresholds.nano-service-max-endpoints}).

The LOC count is computed the microservice detection phase by recursively walking through the service's directory tree and countine lines in all \texttt{.java} files. The endpoint count is populated during the inter-service analysis phase via Spoon-based annotation scanning.
...
\indent \par The God Service detector identifies microservices that handle too many responsibilities \cite{Taibi2018}. Rather than defining responsibility count heuristics directly, this detector uses the output of DesigniteJava's intra-service analysis: specifically, the frequency of \textit{God Class} code smells within a service.
...
\begin{equation}
    \bigl|\{c \in \mathit{classes}(s) : \mathit{isGodClass}(c)\}\bigr| \geq \theta_{\mathit{god}}
    \label{eq:god-service-detection}
\end{equation}

\noindent where $\theta_{\mathit{god}}$ is the minimum number of God Class smells required to flag the service (default: 3, configurable via \texttt{app.thresholds.god-service-min-domains}).

The detector queries the code smell repository for all smells associated with the microservice, filters for those with smell type ``God Class``, and counts the results. When the thresholds is exceeded, code snippets are extracted from the source files of each God Class occurrences, centered on the class declaration or the line number reported by DesigniteJava.
...
\indent \par The Chatty Service detector identifies pairs of services that communicate excessively through fine-grained remote calls \cite{Taibi2018, Richardson2018}. Detection operates on the dependency graph edges, specifically on the \texttt{callCount} attribute annotated on each edge during the inter-service analysis phase.

During the dependency graph construction (see Section~\ref{subsec:graph-analysis}), each time an inter-service call is detected via \texttt{@FeignClient}, \texttt{RestTemplate}, or \texttt{WebClient}, the call count for the corresponding edge is incremented. The Chatty Service detector then queries for all dependency edges where the call count exceeds a configurable threshold:
...
\noindent where $\theta_{\mathit{chatty}}$ is the minimum call count threshold (default: 5, configurable via \texttt{app.thresholds.chatty-service-min-calls}).

For each chatty edge, the detector reports both the source and target services as affected, and extracts code snippets from the evidence file and line number recorded during dependency graph construction. The evidence shows the \texttt{@FeignClient} interface or the \texttt{RestTemplate} invocation responsible for the excessive communication.

\subsection{Hardcoded Endpoint Detection}
\label{subsec:detect-hardcoded-endpoint}

\indent \par The Hardcoded Endpoint detector scans Java code files for URL literals that indicate services are not using a service discovery mechanism \cite{Taibi2020}. The detection algorithm performs a line-by-line scan of all \texttt{.java} files within each service's \texttt{/src/main/java} directory.

The detector uses a configurable set of URL patterns to identify hardcoded endpoints. By default, the patterns include  \texttt{http://}, \texttt{https://}, \texttt{localhost:}, and \texttt{127.0.0.1}. For each line that matches one of these patterns, the detector verifies the match occurs withing a string literal (i.e., enclosed in quotes) and extracts the full URL by use of a regular expression:
...
\begin{itemize}
    \item Comment lines (starting with \texttt{//}, \texttt{*}, or \texttt{/*})
    \item Import statements (\texttt{import ...})
    \item Package declarations (\texttt{package ...})
    \item Test files (paths containing \texttt{test} or \texttt{Test})
\end{itemize}

For each detected hardcoded URL, the detector records the file path, line number, the offending line of code and the extracted URL. Code snippets with surrounding context are read from the source file to provide evidence. The number of evidence snippets is capped at fie per service to avoid excessively large payloads.
...
\indent \par The API Versioning Absence detector checks whether microservices expose REST endpoints without a versioning strategy \cite{Richardson2018}. The detection operates on the endpoint data collected during the Spoon-based inter-service analysis phase.
...
As evidence, the detector locates the controller classes that declare unversioned endpoints and extracts the code snippets showing the \texttt{@RestController} or \texttt{@RequestMapping} annotations. This allows the developer to see where versioning prefixes (e.g., \texttt{/v1/}) could be added.
...
% \indent\par The Wrong Cuts detector identifies misplaced service boundaries where functionality that should reside in one service is spread across multiple services, or tightly-coupled functionality has been incorrectly split \cite{Taibi2018}. The detector uses two complementary indicators.

% \textbf{Indicator 1: Feature Envy Code Smells.} The detector queries the code smell repository for Feature Envy smells within each microservice. Feature Envy is a code smell where a method accesses data or methods of other classes more than its own, suggesting that the method is misplaced \cite{Fowler1999}. When the number of Feature Envy occurrences in a service exceeds a configurable threshold, the service is flagged:
...
% \noindent where $\mathrm{FE}$ denotes the ``Feature Envy'' smell type and $\theta_{\mathit{fe}}$ is the minimum Feature Envy count (default: 3, configurable via \texttt{app.thresholds.wrong-cuts-min-feature-envy}).

% \textbf{Indicator 2: Bidirectional Dependencies.} The detector scans the dependency graph for pairs of services with mutual (bidirectional) dependencies --- that is, edges in both directions between the same pair of vertices. Bidirectional dependencies suggest that the two services are too tightly coupled and may have been incorrectly separated:
...
% \begin{equation}
%     \mathit{medRatio}(s) = \frac{\mathit{inDeps}(s) + \mathit{outDeps}(s)}{2 \times |\mathit{totalDeps}|}
...
% \noindent where $\theta_m$ is the mediator threshold (default: 0.5, configurable via \texttt{app.thresholds.esb-mediator-threshold}). The first condition identifies services that both receive calls from and make calls to a high proportion of other services. The second condition captures cases where a single service concentrates a disproportionate share of all dependency traffic by volume.
