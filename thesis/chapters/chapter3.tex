...
A candidate that does not match any of the three signals is discarded. When the gate eliminates all candidates, three fallback strategies are applied in order: if there was exactly one candidate, it is retained with LOW confidence; if no build files were found at all, the project root is used; otherwise, the project root is used as a catch-all fallback.
...
\noindent where $\theta_{\mathit{chatty}}$ defaults to~5 (configurable via \texttt{app.\-thres\-holds.\-cha\-tty-service-min-calls}). During dependency graph construction, each non-default method in a \texttt{@FeignClient} interface is counted as a separate call (rather than one call per annotation), so a Feign interface with 50~methods produces a call count of~50 on the corresponding edge.
...
All detection thresholds are externalized as configuration parameters, allowing calibration to project-specific norms. Each detector attaches the file path, line number and source snippet that triggered the finding, so a developer can locate the affected code directly. The next chapter describes the implementation of these detection algorithms within the web application.