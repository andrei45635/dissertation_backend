...
\noindent where $\sigma_{st}$ is the total number of shortest paths from $s$ to $t$, and $\sigma_{st}(v)$ is the number of those paths that pass through $v$. To enable threshold comparison across graphs of different sizes, the raw score is normalised by the maximum possible value:

\begin{equation}
    BC_{\mathit{norm}}(v) = \frac{BC(v)}{(n-1)(n-2)}
