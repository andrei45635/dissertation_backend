...
The detection proceeds in two phases. In the \textbf{candidate discovery} phase, the project directory is recursively scanned for build files (\texttt{pom.xml}, \texttt{build.gradle}, \texttt{build\-.gradle\-.kts}). Each directory containing at least one build file becomes a candidate, subject to two filtering steps. First, directories whose name matches an exclusion keyword (e.g.\ \texttt{example}, \texttt{test}, \texttt{benchmark}, \texttt{doc}, \texttt{bom}), or whose ancestor directory does, are discarded. The exclusion set contains over 35~keywords covering examples, samples, documentation, test harnesses, build support modules, starters and templates. Second, aggregator modules are identified and excluded: the project root is treated as an aggregator when it contains \texttt{<modules>} declarations (Maven) or \texttt{settings.gradle} with \texttt{include} statements (Gradle); nested modules that declare their own submodules are likewise excluded.

In the \textbf{deployability gate} phase, each surviving candidate is evaluated against three signals, applied in priority order:

\begin{enumerate}
    \item \textbf{Framework entry point (HIGH confidence).} The module's \texttt{src/main/java} tree is scanned for a class annotated with a known framework startup annotation: \texttt{@SpringBootApplication}, \texttt{@QuarkusMain}, \texttt{@ApplicationPath}, or a call to \texttt{Micronaut.run()}. Files whose names end in \texttt{Application.java} or \texttt{Main.java} are checked first as a fast path before falling back to a full directory scan.

    \item \textbf{Dockerfile presence (MEDIUM confidence).} The module's root directory is checked for a \texttt{Dockerfile}, \texttt{dockerfile}, or any file matching the pattern \texttt{*.Dockerfile}. A Dockerfile implies an independently deployable container, even in the absence of a recognised framework annotation.

    \item \textbf{\texttt{main()} method (LOW confidence).} The module's \texttt{src/main/java} tree is scanned for a \texttt{public static void main(String[])} method signature. This is the weakest signal, as many non-service modules (e.g.\ CLI tools, batch jobs) contain a main method.
\end{enumerate}
...
Each detected service is assigned a \texttt{DetectionConfidence} level (HIGH, MEDIUM, or LOW) and a human-readable signal label (e.g.\ \texttt{framework-entry-point}, \texttt{dockerfile}, \texttt{main-method}), which are persisted alongside the microservice entity. This metadata enables users to understand \textit{why} each module was classified as a service and to assess detection quality.
