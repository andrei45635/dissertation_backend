...
\par Accurate identification of microservice boundaries within a multi-module project is a prerequisite for all downstream analyses. Rather than treating every directory containing a build file as a microservice, the detector applies a deployability gate that consists of a three-signal heuristic that verifies whether a candidate module is an independently deployable service.
...
The first and strongest signal is the presence of a recognised framework startup annotation
within the module's \texttt{src/main/java} tree. The scanner looks for
\texttt{@Spring\-Boot\-Application}, \texttt{@QuarkusMain}, \texttt{@ApplicationPath}, or a
call to \texttt{Micro\-naut.run()}. To keep the process efficient, files whose names end in
\texttt{App\-li\-cation.java} or \texttt{Main.java} are checked first, with a full directory
scan used only as a fallback when no match is found through that faster route. When any of
these annotations is detected, the module is assigned \texttt{HIGH} confidence.

Should no framework annotation be found, the module's root directory is then inspected for
the presence of a \texttt{Dockerfile}, \texttt{dockerfile}, or any file matching the pattern
\texttt{*.Dockerfile}. A Dockerfile indicates that the module is intended to be packaged and
deployed as a self-contained container, which is a strong practical indicator of a deployable
service even in the absence of a recognised framework annotation. This check carries
\texttt{MEDIUM} confidence.

The weakest of the three signals is the presence of a \texttt{public static void main(String[])}
method signature, detected by scanning the module's \texttt{src/main/java} tree. This
heuristic is treated with caution because a main method alone is far from conclusive, as many
modules that are not services at all, such as CLI tools or batch jobs, will naturally contain
one. Accordingly, this signal is assigned \texttt{LOW} confidence.
...
\noindent where $\theta_{\mathit{chatty}}$ defaults to~10 (configurable via \texttt{app.\-thres\-holds.\-cha\-tty-ser\-vice\--min\--calls}). During dependency graph construction, each non-default method in a \texttt{@FeignClient} interface is counted as a separate call (rather than one call per annotation), so a Feign interface with 50~methods produces a call count of~50 on the corresponding edge.
...
\noindent where $\mathrm{FE}$ denotes the ''Feature Envy'' smell type and $\theta_{\mathit{fe}}$ is the minimum Feature Envy count (default: 3, configurable via \texttt{app\-.thres\-holds.\-wrong-cuts\--min\--feat\-ure\--envy}).
...
Wrong Cuts detection combines Feature Envy frequency with bidirectional dependency detection (Section~\ref{subsec:detect-wrong-cuts}). Finally, ESB Misuse detection uses a three-signal approach combining connection ratios, volume-based mediator ratio, and normalised betweenness centrality (Section~\ref{subsec:detect-esb-misuse}).
